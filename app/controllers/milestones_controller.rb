class MilestonesController < ApplicationController
  ...
  # GET /milestones.json
  def index
    @upcoming_milestones = Milestone.find(:all, :order => "finished_by ASC", :conditions => {:status => '1'})
    @completed_milestones = Milestone.find(:all, :order => "finished_by DESC", :conditions => {:status => '2'})
